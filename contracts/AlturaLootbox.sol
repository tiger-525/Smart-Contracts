...
contract AlturaLootbox is ERC1155Holder, VRFConsumerBase  {
    ...
    function initialize(string memory _name, 
        ...
        _staffAccountSet.add(_owner);
         ...
         _salt = uint256(keccak256(abi.encodePacked(_currency, block.timestamp))).mod(10000);
    }
    ...
    function _transferToken(address player, uint256 amount) private {
        currencyToken.transferFrom(player, paymentAddress, amount);
    }


    function _distributePrize(address player) private {
        for (uint i = 0; i < gameRounds[player].times; i ++) {
    ...
    function changeTokenAddress(address _currency) external onlyOwner {
        currencyToken = IERC20(_currency);
    }
